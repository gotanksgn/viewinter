...
import com.gotanks.uni_alisv.editor.Transcoder;
import com.gotanks.uni_alisv.editor.bean.AlivcEditInputParam;
import com.gotanks.uni_alisv.media.MutiMediaView;
...
public class EditorMediaActivity extends Activity {
    ...
    private void init() {
        ...
        //小于10秒的不让选择  超过最大长度的 进行裁剪
        mMutiMediaView.setVideoDurationRange(AliSvWXModule.MIN_TIME, Integer.MAX_VALUE);
        mMutiMediaView.enableSelectView(AliSvWXModule.MAX_TIME);
        ...
        mMutiMediaView.setOnActionListener(new MutiMediaView.OnActionListener() {
            ...
            public void onNext(boolean isReachedMaxDuration) {
                ...
                if (videoCount > 0 && (progressDialog == null || !progressDialog.isShowing())) {
                    ...
                    //单选 取出第0个
                    MediaInfo mediaInfo = mTransCoder.getOriginalVideos().get(0);
                    //超出的时长，进行裁剪
                    if (mediaInfo.duration >= AliSvWXModule.MAX_TIME) {
                        ToastUtil.showToast(getApplicationContext(), String.format("视频时长大于%s秒，请裁剪", AliSvWXModule.MAX_TIME / 1000));
                        startCropVideo(mediaInfo, 0);
                        return;
                    }
                    ...
                    mTransCoder.transcode(getApplicationContext(), mInputParam.getScaleMode());
                } else {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            if (requestCode == AliyunVideoCropActivity.REQUEST_CODE_EDITOR_VIDEO_CROP) {
    ...
    public void startCropVideo(MediaInfo info, int position) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && !TextUtils.isEmpty(info.fileUri)) {
            //适配 android Q, copy 媒体文件到应用沙盒下
            info.filePath = cacheMediaFile(EditorMediaActivity.this, info.fileUri, info.filePath);
        }
        ...
        AlivcCropInputParam cropInputParam = new AlivcCropInputParam.Builder()
                ...
                .setMinCropDuration(AliSvWXModule.MIN_TIME)
