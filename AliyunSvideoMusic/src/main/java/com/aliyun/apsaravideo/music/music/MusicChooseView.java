...
public class MusicChooseView extends LinearLayout implements View.OnClickListener {
    ...
    protected void onAttachedToWindow() {
        ...
        if (isShowed && mCacheMusic != null && mMusicAdapter != null) {
            ...
            try {
                prepareMusicInfo(mCacheMusic, mCacheMusic.path);
            } catch (IOException e) {
                e.printStackTrace();
            }
     ...
     * 准备要播放的音乐资源
     */
    private void prepareMusicInfo(MusicFileBean musicFileBean, String path) throws IOException, IllegalStateException {
        mPlayHandler.removeCallbacks(mMusciRunnable);
        mMediaPlayer.reset();
